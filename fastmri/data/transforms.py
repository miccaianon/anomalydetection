...
#         shape = [data.shape[-2],data.shape[-2]] 
    ...
    w_to = w_from + shape[0]
    h_to = h_from + shape[1]
    # print(data.shape, shape, w_from, h_from, w_to, h_to)
...
class UnetDataTransform:
    ...
    def __call__(
        ...
        if self.crop_all:
            image = torch.nn.functional.interpolate(image,size=(320,320), mode= 'bilinear')
        image= torch.squeeze(torch.squeeze(image, dim =0), dim=0 )
        ...
        if self.crop_all:
            target = torch.nn.functional.interpolate(target,size=(320,320), mode= 'bilinear')
        target= torch.squeeze(torch.squeeze(target, dim =0), dim=0 )
