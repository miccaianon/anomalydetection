...
class SliceDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        dataset_cache_file: Union[str, Path, os.PathLike] = "dataset_cache.pkl",
        num_cols: Optional[Tuple[int]] = None,
        anns_path: Union[str, Path, os.PathLike] =None,
        maps_path: Union[str, Path, os.PathLike] =None,
        num_classes: int =3, 
        mode: str = None,
        load_sens= False
    ):
        ...
        Args:
            ...
            mode: get annotations for the type of annotations
        """
        ...
        self.ext = list(self.anns.keys())[0][-4:] #file extension, just because the annotations are saved like this
        self.mode = mode
        ...
        if dataset_cache.get(root) is None or not use_dataset_cache:
            ...
            for fname in sorted(files):
                ...
                for slc in range(num_slices):
                    if slc < 13:
                        ann = str(fname.stem)+ "_"+ str(slc) + self.ext

                        if self.mode == 'fastmri-binary':
                            if ann in self.anns.keys():
                                valid_slices.append(int(ann[:-4].split('_')[5]))

                        else: 
                            if self.anns[ann] != 1: #not relevant
                                valid_slices.append(int(ann[:-4].split('_')[5]))
                        
                self.examples += [
                    (fname, slice_ind, metadata) for slice_ind in valid_slices
                ]
    ...
    def __getitem__(self, i: int):
        ...
        label = self.anns[fname.stem+ "_"+ str(dataslice) + self.ext]
        
        if self.load_sens:
            smapfilename = self.maps_path /  f'{fname.stem}_{dataslice}.npy'  
            smap = torch.from_numpy(np.load(smapfilename))

        if self.transform is None:
            sample = (kspace, mask, target, attrs, fname.name, dataslice, label, smap)
        else:
